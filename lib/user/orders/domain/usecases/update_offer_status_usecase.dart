import 'package:dartz/dartz.dart';
import 'package:decorizer/common/base/usecase.dart';
import 'package:decorizer/common/failure.dart';
import 'package:decorizer/user/orders/domain/params/update_offer_status_params.dart';
import 'package:decorizer/user/orders/domain/repositories/user_orders_repository.dart';
import 'package:injectable/injectable.dart';

@Injectable()
class UpdateOfferStatusUsecase extends UseCase<Unit, UpdateOfferStatusParams> {
  final UserOrdersRepository userOrdersRepository;

  UpdateOfferStatusUsecase({required this.userOrdersRepository});

  @override
  Future<Either<Failure, Unit>> call(UpdateOfferStatusParams params) async {
    return userOrdersRepository.updateOfferStatus(params);
  }
}
