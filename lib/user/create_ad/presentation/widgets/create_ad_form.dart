...
import 'package:decorizer/user/create_ad/presentation/pages/create_ad_screen.dart';
import 'package:decorizer/user/create_ad/presentation/widgets/create_ad_drop_downs_section.dart';
import 'package:decorizer/user/create_ad/presentation/widgets/location_on_map_section.dart';
import 'package:decorizer/user/create_ad/presentation/widgets/pick_design_section.dart';
...
class CreateAdForm extends StatelessWidget {
      ...
      child: SingleChildScrollView(
        child: Container(
          ...
          child: Column(
            ...
            children: <Widget>[
              const CreateAdDropDownsSection(),
              Height(12.h),
              AppTextField(
              ...
              Height(16.h),
              const LocationOnMapSection(),
              Height(23.h),
              BlocConsumer<CreateAdCubit, CreateAdState>(
                listener: (context, state) => state.createAdState.listen(
                  onSuccess: (data, message) {
                    _onCreateAdSuccess(context);
                  },
                  onError: showErrorToast,
                ),
                builder: (context, state) => AppButton(
                  isLoading: state.createAdState.isLoading,
                  fontSize: 14.sp,
                  backgroundColor: context.appColors.primary,
                  rippleColor: context.appColors.splashColor,
                  text: context.tr(LocaleKeys.action_create),
                  onClick: () {
                    _onCreateAdClicked(context);
                  },
                ),
              )
            ],
  ...
  void _onCreateAdClicked(BuildContext context) {
    final cubit = context.read<CreateAdCubit>();
  ...
  void _onCreateAdSuccess(BuildContext context) {
